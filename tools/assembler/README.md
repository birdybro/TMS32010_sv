...
The realistic-program workflow additionally assembles
`sim/programs/fir4/fir4.asm`, checks its words against an independently fixed
image, round-trips that image through the disassembler, and executes it in the
architectural model. The regression fixes the Q15 result, sample-history
movement, twelve documented instruction cycles, one logical program fetch per
instruction, and every logical data transaction in
`sim/programs/fir4/expected.json`.
