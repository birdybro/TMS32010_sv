    ...
    def disassemble_word(self, word: int) -> str:
        ...
        if mnemonic == "SACL":
            ...
            if control & 0x08:
                if control & 1:
                    return f".word 0x{word:04x}"
                return f"SACL {modifier}"
            return f"SACL {modifier},0,{control & 1}"
        if mnemonic == "SACH":
            shift = operands["shift"]
            if not operands["indirect"]:
                suffix = f",{shift}" if shift else ""
                return f"SACH {operands['addressing_field']}{suffix}"
            control = operands["addressing_field"]
            modifier = {0x00: "*", 0x20: "*+", 0x10: "*-"}[control & 0x30]
            if control & 0x08:
                if control & 1:
        ...
        if mnemonic in {"LAR", "SAR"}:
            register = operands["auxiliary_register"]
            if not operands["indirect"]:
                return (
                    f"{mnemonic} AR{register},{operands['addressing_field']}"
                )
            control = operands["addressing_field"]
            modifier = {0x00: "*", 0x20: "*+", 0x10: "*-"}[control & 0x30]
            if control & 0x08:
                if control & 1:
                    return f".word 0x{word:04x}"
                return f"{mnemonic} AR{register},{modifier}"
            return f"{mnemonic} AR{register},{modifier},{control & 1}"
        if mnemonic == "LARK":
            return (
