...
recognizes the eleven-instruction model/tool slice. Unknown words are rendered
...
noncanonical indirect `LAC`, `SACL`, or `SACH` whose ignored bit 0 is one while
ARP is preserved also uses `.word`, retaining its exact binary representation.
