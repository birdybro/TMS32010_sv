...
recognizes the thirteen-instruction model/tool slice. Unknown words are rendered
...
noncanonical indirect `LAC`, `SACL`, `SACH`, `ZALH`, or `ZALS` whose ignored
bit 0 is one while ARP is preserved also uses `.word`, retaining its exact
