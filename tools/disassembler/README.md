...
recognizes the ten-instruction model/tool slice. Unknown words are rendered
...
noncanonical indirect `LAC` or `SACL` whose ignored bit 0 is one while ARP is
preserved also uses `.word`, retaining its exact binary representation.
