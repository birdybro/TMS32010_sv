...
// the three represented cycles of one fixed direct TBLR instruction.
//
// This proves logical core sequencing and the portable RAM transaction. It
// does not establish original-package subphase or electrical timing.
module tms32010_interrupt_table_formal (
  input logic clk_i,
  ...
  // the enabled boundary after the opcode, discarded fetch, or table transfer.
  assign int_n =
    !(
      initialized &&
      (
  ...
  always_comb begin
    case (program_address)
      12'h000: program_data = 16'h7f82;  // EINT and TBLR source word
      12'h001: program_data = 16'h6700;  // direct TBLR data address 0
      12'h002: program_data = 16'h2000;  // protected/vector direct LAC 0
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (initialized) begin
      ...
      assert (
        data_read ==
        ((cycle_count == 32'd4) || (cycle_count == 32'd6))
      );
      assert (!(io_read || io_write));
      assert (data_write == (cycle_count == 32'd3));
      ...
      case (cycle_count)
        ...
        32'd3: begin
          // The table-transfer interval reads program 0 and writes RAM 0.
          assert (pc == 12'h002);
          assert (!interrupt_mask);
          ...
          assert (data_write_data == 16'h7f82);
          assert (stack_top == 12'h000);
        end
        32'd4: begin
          // TBLR has completed before interrupt deferral begins.
          assert (pc == 12'h002);
          assert (accumulator == 32'h0000_0000);
          assert (!interrupt_mask);
          assert (interrupt_pending);
          ...
          assert (data_address_valid);
          assert (data_address == 8'h00);
          assert (data_read_data == 16'h7f82);
          assert (stack_top == 12'h000);
        ...
        32'd5: begin
          assert (pc == 12'h003);
          assert (accumulator == 32'h0000_7f82);
          assert (!interrupt_mask);
          assert (interrupt_pending);
          assert (!instruction_valid);
