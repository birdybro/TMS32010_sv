...
// FORMAL-001 bounded harness for the qualified interrupt-entry slice.
//
// The program is deliberately fixed to the primary-backed sequence
// EINT, protected LACK, dummy return-PC fetch, and vector LACK. Clock enable
// remains arbitrary so every state can stall for any number of bounded steps.
// See formal/README.md for the exact assumptions and excluded claims.
module tms32010_interrupt_formal (
  input logic clk_i,
  ...
  // Hold INT active until EINT at address zero is accepted. Thereafter the
  // level is inactive; the internally latched request must preserve the pulse.
  assign int_n = !(initialized && (cycle_count == 32'd0));

  always_comb begin
    case (program_address)
      12'h000: program_data = 16'h7f82;  // EINT
      12'h001: program_data = 16'h7e2a;  // protected LACK 0x2a
      12'h002: program_data = 16'h7e5a;  // dummy word, then vector instruction
  ...
  always_ff @(posedge clk_i) begin
    ...
    if (initialized) begin
      ...
      case (cycle_count)
        ...
        32'd2: begin
          assert (pc == 12'h002);
          assert (accumulator == 32'h0000_002a);
          assert (!interrupt_mask);
          assert (interrupt_pending);
          assert (!instruction_valid);
        ...
        32'd4: begin
          assert (pc == 12'h003);
          assert (accumulator == 32'h0000_005a);
          assert (stack_top == 12'h002);
          assert (interrupt_mask);
          assert (!interrupt_pending);
        end
