...
The reference model, local tools, and partial RTL currently support twenty
instructions: `ADD`, `ADDS`, `AND`, `LAC`, `LACK`, `LARK`, `LARP`, `LDPK`,
`NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `SUB`, `SUBS`, `XOR`, `ZAC`,
`ZALH`, and `ZALS`. The
144-word internal RAM exposes verification-visible logical
`ADD`/`ADDS`/`AND`/`LAC`/`OR`/`SUB`/`SUBS`/`XOR`/`ZALH`/`ZALS` reads and `SACL`/`SACH` writes;
...
this twenty-instruction subset only; it is not a general
