...
The reference model, local tools, and partial RTL currently support forty-five
instructions: `ADD`, `ADDS`, `AND`, `APAC`, `B`, `BANZ`, `BGEZ`, `BGZ`, `BLEZ`, `BLZ`, `BNZ`, `BZ`, `DINT`, `DMOV`, `EINT`, `LAC`, `LACK`, `LAR`, `LARK`, `LARP`,
...
instructions and the eight qualified two-cycle branches; it is not a general pipeline or
