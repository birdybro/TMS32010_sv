...
read relationship for the fourteen supported one-cycle sequential
instructions. Its `ADDS`, `LAC`, `ZALH`, and `ZALS` cases expose concurrent
internal logical reads, while `SACL` and `SACH` expose writes, without
