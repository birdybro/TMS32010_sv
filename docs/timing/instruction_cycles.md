...
program-read cycle for every instruction in the nineteen-instruction subset,
then checks retirement on the falling-edge sample boundary. Directed `ADD`, `ADDS`,
`AND`, `LAC`, `OR`, `SACL`, `SACH`, `SUB`, `XOR`, `ZALH`, and `ZALS` RTL tests
separately check one
architectural cycle for direct and indirect cases, including every documented
SACH shift, positive and negative ADD/SUB saturation, both ADDS overflow-mode
...
- interaction of program fetch with internal data RAM beyond the qualified
  one-cycle `ADD`/`ADDS`/`AND`/`LAC`/`OR`/`SUB`/`XOR`/`ZALH`/`ZALS` reads and
  `SACL`/`SACH` writes;
