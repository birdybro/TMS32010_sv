...
program-read cycle for every instruction in the fourteen-instruction subset,
then checks retirement on the falling-edge sample boundary. Directed `ADDS`,
`LAC`, `SACL`, `SACH`, `ZALH`, and `ZALS` RTL tests separately check one
architectural cycle for direct and indirect cases, including every documented
SACH shift and both ADDS overflow-mode outcomes. This qualifies the documented
...
- interaction of program fetch with internal data RAM beyond the qualified
  one-cycle `ADDS`/`LAC`/`ZALH`/`ZALS` reads and `SACL`/`SACH` writes;
