...
`ZAC`, `ROVM`, and `SOVM` at an instruction-boundary program interface. One
asserted `clock_enable_i` retires one supported one-cycle instruction.
Unsupported words and `LAC` accesses outside the verified 144-word RAM assert
`illegal_o` and do not advance the PC.
...
physical reset never initializes the RAM, and an assertion rejects preload
during live execution. Logical data address/read/data outputs expose internal
...
For the nine currently qualified one-cycle sequential instructions it samples
...
multi-cycle, data-memory writes, I/O, table, and interrupt sequences remain
absent.
