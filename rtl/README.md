...
`ROVM`, `SACL`, `SOVM`, and `ZAC` at an instruction-boundary program
interface. One asserted `clock_enable_i` retires one supported one-cycle
instruction. Unsupported words and `LAC`/`SACL` accesses outside the verified
...
live execution or collision with the architectural `SACL` write. Logical data
...
For the ten currently qualified one-cycle sequential instructions it samples
