...
`LDPK`, `NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `SUB`, `XOR`, `ZAC`, `ZALH`,
and `ZALS` at an
...
For the nineteen currently qualified one-cycle sequential instructions it
...
- exposes sticky `OV` for ADD/ADDS/SUB wrap/saturation verification;
