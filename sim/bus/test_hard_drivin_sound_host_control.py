...
class HardDrivinSoundHostControlTests(unittest.TestCase):
    def test_address_encoded_latches_reset_and_validity(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_sound_host_control"
