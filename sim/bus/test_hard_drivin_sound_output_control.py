...
class HardDrivinSoundOutputControlTests(unittest.TestCase):
    def test_exhaustive_mute_data_irq_data_independence_and_reset(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_sound_output_control"
