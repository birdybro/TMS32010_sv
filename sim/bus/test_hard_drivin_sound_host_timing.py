...
class HardDrivinSoundHostTimingTests(unittest.TestCase):
    def test_rvf_dtack_rvas_and_low_io_phase_sequence(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_sound_host_timing"
