...
module tb_phase_slice_integration;
  ...
  initial begin
    ...
    require(!data_read && !data_write && !data_address_valid,
            "APAC has only its normal program-memory transaction");

    advance_to_sample();
    require(retired && accumulator == 32'h0004_9332,
            "APAC adds the complete P register to the accumulator");
    require(product_register == 32'h0002_4999 &&
            t_register == 16'hbeef &&
            !overflow_flag && overflow_mode,
            "APAC preserves P, T, and nonoverflowing arithmetic status");
    require(pc == 12'h01f && cycle_count == 32'd31,
            "APAC consumes one native instruction cycle");
    require(!data_read && !data_write && !data_address_valid,
            "SPAC has only its normal program-memory transaction");

    advance_to_sample();
    require(retired && accumulator == 32'h0002_4999,
            "SPAC subtracts the complete P register from the accumulator");
    require(product_register == 32'h0002_4999 &&
            t_register == 16'hbeef &&
            !overflow_flag && overflow_mode,
    ...
    require(data_read && data_write && data_address_valid &&
            data_address == 8'h03 && data_read_data == 16'hbeef,
            "LTD presents its source read beside normal program phases");
    require(data_write_address_valid && data_write_address == 8'h04 &&
            data_write_data == 16'hbeef,
            "LTD exposes its distinct next-address copy transaction");

    advance_to_sample();
    require(retired && accumulator == 32'h0006_dccb &&
            t_register == 16'hbeef,
            "LTD loads T while accumulating the previous P value");
    require(product_register == 32'h0002_4999 &&
            dut.core.data_ram.memory[4] == 16'hbeef,
            "LTD preserves P and copies the source word to the next location");
    require(!overflow_flag && overflow_mode,
    ...
    require(data_read && !data_write && data_address_valid &&
            data_address == 8'h03 && data_read_data == 16'hbeef,
            "SUBH exposes its high-half operand beside normal program phases");
