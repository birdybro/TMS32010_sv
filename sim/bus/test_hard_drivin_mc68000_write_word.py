...
class HardDrivinMc68000WriteWordTests(unittest.TestCase):
    def test_every_word_and_byte_lane_matches_the_primary_bus_table(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_mc68000_write_word"
