...
class HardDrivinMainHsbusTimingTests(unittest.TestCase):
    def test_primary_transcribed_early_hsbus_acknowledgement(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_main_hsbus_timing"
