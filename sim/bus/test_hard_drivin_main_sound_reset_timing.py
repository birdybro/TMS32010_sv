...
class HardDrivinMainSoundResetTimingTests(unittest.TestCase):
    def test_primary_transcribed_complete_reset_write_sequence(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_main_sound_reset_timing"
        ...
        sources = [
            ...
            / "rtl"
            / "wrappers"
            / "hard_drivin_main_sound_reset_decode.sv",
