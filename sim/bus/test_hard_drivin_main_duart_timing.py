...
class HardDrivinMainDuartTimingTests(unittest.TestCase):
    def test_primary_qualified_external_duart_acknowledgement(self) -> None:
        verilator = shutil.which("verilator")
        if verilator is None:
            raise RuntimeError("Verilator is required once architectural RTL exists")
        name = "tb_hard_drivin_main_duart_timing"
