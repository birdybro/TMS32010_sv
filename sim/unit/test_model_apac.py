...
class ApacModelTests(unittest.TestCase):
    def _execute(
        self,
        accumulator: int,
        product: int,
        *,
        ...
        model.program[0] = 0x7F8F

        trace = model.step()

        self.assertEqual(trace.mnemonic, "APAC")
    ...
    def test_primary_example_adds_product_to_accumulator(self) -> None:
        model = self._execute(0x0000_0020, 0x0000_0040)

        self.assertEqual(model.state.acc, 0x0000_0060)
    ...
    def test_positive_overflow_wraps_when_ovm_is_clear(self) -> None:
        model = self._execute(0x7FFF_FFFE, 0x0000_0002)
    ...
    def test_negative_overflow_wraps_when_ovm_is_clear(self) -> None:
        model = self._execute(0x8000_0001, 0xFFFF_FFFE)
    ...
    def test_nonoverflowing_addition_preserves_sticky_ov_and_other_state(self) -> None:
        model = Tms32010Model()
        model.state.acc = 0xFFFF_FFF0
        ...
        self.assertEqual(model.state.acc, 0xFFFF_FFF8)
