...
module tb_spac_rtl;
  ...
  initial begin
    ...
    program_memory[1]  = 16'h8024;  // MPYK 36 -> P=36
    program_memory[2]  = 16'h7e3c;  // LACK 60
    program_memory[3]  = 16'h7f90;  // SPAC -> 24
    program_memory[4]  = 16'h6a02;  // LT 2
    program_memory[5]  = 16'h9fc0;  // MPYK -64
    program_memory[6]  = 16'h6500;  // ZALH 0 -> 0x7fff0000
    program_memory[7]  = 16'h6101;  // ADDS 1 -> 0x7ffffffe
    program_memory[8]  = 16'h7f90;  // SPAC positive wrap
    program_memory[9]  = 16'h7f8b;  // SOVM
    program_memory[10] = 16'h6500;  // ZALH 0
    program_memory[11] = 16'h6101;  // ADDS 1
    program_memory[12] = 16'h7f90;  // SPAC positive saturation
    program_memory[13] = 16'h6a02;  // LT 2
    program_memory[14] = 16'h8040;  // MPYK 64
    program_memory[15] = 16'h7f8a;  // ROVM
    program_memory[16] = 16'h6503;  // ZALH 3 -> 0x80000000
    program_memory[17] = 16'h6102;  // ADDS 2 -> 0x80000001
    program_memory[18] = 16'h7f90;  // SPAC negative wrap
    program_memory[19] = 16'h7f8b;  // SOVM
    program_memory[20] = 16'h6503;  // ZALH 3
    program_memory[21] = 16'h6102;  // ADDS 2
    program_memory[22] = 16'h7f90;  // SPAC negative saturation
    program_memory[23] = 16'h7f91;  // unsupported control word
    ...
    tick();
    tick();
    require(product_register == 32'd36,
            "MPYK establishes TI's example P operand");
    tick();
    require(accumulator == 32'd60 &&
            instruction_valid && program_read &&
            program_address == 12'd3,
            "primary example reaches SPAC with ACC=60 and P=36");
    require(!data_read && !data_write && !data_address_valid,
            "SPAC has no logical data-memory transaction");

    tick();
    require(retired && accumulator == 32'd24 &&
            product_register == 32'd36,
            "SPAC primary example produces 24 and preserves P");
    require(!overflow_flag && !overflow_mode,
            "nonoverflowing SPAC leaves clear OV and OVM unchanged");
    tick();
    tick();
    ...
    require(accumulator == 32'h7fff_fffe &&
            ...
            "positive boundary reaches SPAC through program-only activity");

    tick();
    require(accumulator == 32'h8000_003e &&
            overflow_flag && !overflow_mode,
            "SPAC wraps positive overflow and sets sticky OV when OVM is clear");
    tick();
    tick();
    tick();
    require(accumulator == 32'h7fff_fffe &&
    ...
    require(!data_read && !data_write && !data_address_valid,
            "saturating SPAC remains program-only");

    tick();
    require(accumulator == 32'h7fff_ffff &&
            product_register == 32'hffff_ffc0,
            "SPAC saturates positive overflow and preserves P");
    tick();
    tick();
    ...
    require(accumulator == 32'h8000_0001 &&
            overflow_flag && !overflow_mode,
            "negative boundary reaches SPAC with OVM clear");
    require(!data_read && !data_write && !data_address_valid,
            "negative-overflow SPAC has no data transaction");

    tick();
    require(accumulator == 32'h7fff_ffc1 &&
            product_register == 32'h0000_0040 &&
            overflow_flag && !overflow_mode,
            "SPAC wraps negative overflow and preserves P");
    tick();
    tick();
    tick();
    require(accumulator == 32'h8000_0001 &&
    ...
    require(accumulator == 32'h8000_0000 &&
            product_register == 32'h0000_0040,
            "SPAC saturates negative overflow to the signed endpoint");
    require(overflow_flag && overflow_mode &&
            t_register == 16'h0001,
            "SPAC preserves P, T, sticky OV, and OVM");
    require(auxiliary_register_0 == 16'h0000 &&
            auxiliary_register_1 == 16'h0000 &&
            !auxiliary_register_pointer && !data_page_pointer,
            "SPAC leaves address state unchanged");
    require(pc == 12'd23 && cycle_count == 32'd23,
            "every SPAC consumes exactly one instruction cycle");
    ...
    require(illegal && !retired && pc == 12'd23 &&
            cycle_count == 32'd23,
            "unsupported word traps without changing qualified SPAC state");
    require(accumulator == 32'h8000_0000 &&
            product_register == 32'h0000_0040,
            "trap preserves the final SPAC result");
