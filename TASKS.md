...
- **Notes:** Nine encodings (`LAC`, `LACK`, `LARK`, `LARP`, `LDPK`, `NOP`,
  `ZAC`, `ROVM`, `SOVM`) are primary-transcribed in the opcode research table.
  `LAC` adds conditional legality constraints for indirect control bits. The
  decoder exhaustively classifies all 65,536 words against this partial set
  without collisions; the remaining 51 instructions and full reserved-region
...
- **Notes:** Independent model supports `LAC`, `LACK`, `LARK`, `LARP`, `LDPK`,
  `NOP`, `ZAC`, `ROVM`, and `SOVM`, raw program loading, logical program/data
  traces, reset-boundary effects, and deterministic replay. `LAC` covers
  direct/indirect reads, sign extension, shifts, and nine-bit AR updates;
...
- **Notes:** Qualified slice supports the same nine instructions as the model,
  labels, expressions, `.word`, `.org`, `.include`, raw/hex/listing output,
  lossless unknown-word disassembly, and round trips. `LAC` supports checked
  direct and indirect TI syntax. The remaining 51
