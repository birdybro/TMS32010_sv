...
- **Notes:** Seventeen encodings (`ADDS`, `AND`, `LAC`, `LACK`, `LARK`, `LARP`,
  `LDPK`, `NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `XOR`, `ZAC`, `ZALH`,
  `ZALS`) are primary-transcribed in the opcode research table. The nine
  ...
  without collisions; the remaining 43 instructions and full reserved-region
...
- **Notes:** Independent model supports `ADDS`, `AND`, `LAC`, `LACK`, `LARK`,
  `LARP`, `LDPK`, `NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `XOR`, `ZAC`,
  `ZALH`, and `ZALS`,
  raw program loading, logical program/data traces, reset-boundary effects,
  and deterministic replay. The nine common-address data instructions cover
...
- **Notes:** Qualified slice supports the same seventeen instructions as the
  ...
  ADDS/AND/OR/XOR/ZALH/ZALS common address syntax without a shift operand. The
  remaining 43 documented instructions are rejected explicitly. A surviving
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OV/OVM, and
  144-word internal RAM exist for the seventeen-instruction slice. `LAC`
  ...
  nine common-address data instructions verify direct/indirect read/write
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of seventeen supported one-cycle instructions, including all
  nine internal-data operations, on its falling-edge sample and aligns
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all seventeen
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `ADDS`/`AND`/`LAC`/`OR`/`SACL`/`SACH`/`XOR`/`ZALH`/`ZALS` model/RTL tests
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all nine qualified
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  ...
  `ADDS`/`AND`/`LAC`/`OR`/`XOR`/`ZALH`/`ZALS` reads,
...
- **Notes:** Seventeen-instruction RTL, phase engine, and 144-word RAM are
