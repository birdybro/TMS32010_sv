...
- **Notes:** Eight encodings (`LACK`, `LARK`, `LARP`, `LDPK`, `NOP`, `ZAC`,
  `ROVM`, `SOVM`) are primary-transcribed in the opcode research table. The
  decoder exhaustively classifies all 65,536 words against this partial set
  without collisions; the remaining 52 instructions and full reserved-region
...
- **Notes:** Independent first slice supports `LACK`, `LARK`, `LARP`, `LDPK`,
  `NOP`, `ZAC`, `ROVM`, and `SOVM`, raw program loading, logical fetch traces,
  reset-boundary effects, and deterministic replay. Unsupported words trap.
  Interrupts, memory/I/O instructions, and pin phases remain unimplemented.
  Keep model structure independent of the future RTL.
...
- **Notes:** Qualified slice supports the same eight instructions as the model,
  labels, expressions, `.word`, `.org`, `.include`, raw/hex/listing output,
  lossless unknown-word disassembly, and round trips. The remaining 52
