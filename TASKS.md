...
- **Notes:** Eleven encodings (`LAC`, `LACK`, `LARK`, `LARP`, `LDPK`, `NOP`,
  `ROVM`, `SACL`, `SACH`, `SOVM`, `ZAC`) are primary-transcribed in the opcode
  research table. `LAC`, `SACL`, and `SACH` add conditional legality
  constraints for indirect control bits; SACH additionally restricts its
  sparse shift field to 0, 1, and 4. The decoder exhaustively classifies all
  65,536 words against this partial set without collisions; the remaining 49
...
- **Notes:** Independent model supports `LAC`, `LACK`, `LARK`, `LARP`, `LDPK`,
  `NOP`, `ROVM`, `SACL`, `SACH`, `SOVM`, and `ZAC`, raw program loading, logical
  program/data traces, reset-boundary effects, and deterministic replay. `LAC`,
  `SACL`, and `SACH` cover direct/indirect reads/writes and nine-bit AR updates;
  `LAC` additionally covers sign extension and shifts, while SACH covers
  output shifts 0/1/4. Out-of-range original-RAM addresses and unsupported
  words trap. Interrupts,
...
- **Notes:** Qualified slice supports the same eleven instructions as the model,
  ...
  placeholder before a next ARP and SACH's sparse 0/1/4 shifts. The remaining
  49 documented instructions are rejected explicitly. A surviving binary tool
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OVM, and
  144-word internal RAM exist for the eleven-instruction slice. `LAC` verifies
  sign extension and left shifts; `SACH` verifies its output-shifter
  cross-half behavior; `LAC`/`SACL`/`SACH` verify direct/indirect read/write
  addressing and low-nine-bit AR updates. Reset deliberately
  preserves OVM and does not invent ACC/AR/ARP/DP or RAM power-up state. ALU,
  multiplier, other output-shifter consumers, stack, and remaining status
  behavior remain.
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of eleven supported one-cycle instructions, including internal
  `LAC`/`SACL`/`SACH`, on its falling-edge sample and aligns PC/native address across
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all eleven
  supported sequential instructions, checks that internal `LAC`/`SACL`/`SACH`
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `LAC`/`SACL`/`SACH` model/RTL tests cover valid address selection, logical
...
- **Notes:** First control/immediate slice (`LACK`, `NOP`, `ZAC`, `ROVM`,
  ...
  The rest of the load/store family remains. Maintain one
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle `LAC`, `SACL`, and `SACH` retirement is asserted
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  state, logical-cycle, `LAC` read, `SACL`/`SACH` writes, and final 144-word RAM
...
- **Notes:** Eleven-instruction RTL, phase engine, and 144-word RAM fit Cyclone V
  in 1,380 ALMs/2,420 registers with no RAM/DSP blocks, 76.88 MHz worst
  ...
  structural/generic synthesis in isolated Ubuntu 24.04 with 6,189 generic
  cells, lowering the asynchronous RAM to flip-flops/muxes. Full-core
