...
- **Notes:** Eighteen encodings (`ADD`, `ADDS`, `AND`, `LAC`, `LACK`, `LARK`, `LARP`,
  `LDPK`, `NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `XOR`, `ZAC`, `ZALH`,
  `ZALS`) are primary-transcribed in the opcode research table. The ten
  ...
  without collisions; the remaining 42 instructions and full reserved-region
...
- **Notes:** Independent model supports `ADD`, `ADDS`, `AND`, `LAC`, `LACK`, `LARK`,
  ...
  and deterministic replay. The ten common-address data instructions cover
  ...
  positive/negative saturation endpoints.
...
- **Notes:** Qualified slice supports the same eighteen instructions as the
  ...
  ADD/LAC common address syntax with shifts and
  ADDS/AND/OR/XOR/ZALH/ZALS syntax without a shift operand. The remaining 42
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OV/OVM, and
  144-word internal RAM exist for the eighteen-instruction slice. `LAC`
  ...
  ten common-address data instructions verify direct/indirect read/write
  ...
  overflow state. ADD verifies signed shifting, general signed overflow,
  wrap, and both saturation endpoints. Physical reset
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of eighteen supported one-cycle instructions, including all
  ten internal-data operations, on its falling-edge sample and aligns
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all eighteen
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `ADD`/`ADDS`/`AND`/`LAC`/`OR`/`SACL`/`SACH`/`XOR`/`ZALH`/`ZALS` model/RTL tests
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all ten qualified
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  ...
  `ADD`/`ADDS`/`AND`/`LAC`/`OR`/`XOR`/`ZALH`/`ZALS` reads,
...
- **Notes:** Eighteen-instruction RTL, phase engine, and 144-word RAM are
