...
- **Notes:** Fourteen encodings (`ADDS`, `LAC`, `LACK`, `LARK`, `LARP`,
  `LDPK`, `NOP`, `ROVM`, `SACL`, `SACH`, `SOVM`, `ZAC`, `ZALH`, `ZALS`) are
  primary-transcribed in the opcode research table. The six common-address
  ...
  without collisions; the remaining 46 instructions and full reserved-region
...
- **Notes:** Independent model supports `ADDS`, `LAC`, `LACK`, `LARK`, `LARP`,
  `LDPK`, `NOP`, `ROVM`, `SACL`, `SACH`, `SOVM`, `ZAC`, `ZALH`, and `ZALS`,
  raw program loading, logical program/data traces, reset-boundary effects,
  and deterministic replay. The six common-address data instructions cover
...
- **Notes:** Qualified slice supports the same fourteen instructions as the
  ...
  ADDS/ZALH/ZALS common address syntax without a shift operand. The remaining 46
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OV/OVM, and
  144-word internal RAM exist for the fourteen-instruction slice. `LAC` verifies
  ...
  six common-address data instructions verify direct/indirect read/write
  addressing and low-nine-bit AR updates. ADDS additionally verifies sticky
  overflow, OVM-clear wrap, and OVM-set positive saturation. Physical reset
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of fourteen supported one-cycle instructions, including all
  six internal-data operations, on its falling-edge sample and aligns
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all fourteen
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `ADDS`/`LAC`/`SACL`/`SACH`/`ZALH`/`ZALS` model/RTL tests cover valid address
...
- **Notes:** First control/immediate slice (`LACK`, `NOP`, `ZAC`, `ROVM`,
  ...
  `ADDH` remains explicitly unimplemented under `SC-006`/`OQ-011`. The rest
  of the arithmetic and load/store families remain. Maintain one subtask per
  family when implementation begins.
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all six qualified
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  state including OV/OVM, logical-cycle, `ADDS`/`LAC`/`ZALH`/`ZALS` reads,
...
- **Notes:** Fourteen-instruction RTL, phase engine, and 144-word RAM are
