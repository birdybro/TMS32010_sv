...
- **Notes:** Nineteen encodings (`ADD`, `ADDS`, `AND`, `LAC`, `LACK`, `LARK`, `LARP`,
  `LDPK`, `NOP`, `OR`, `ROVM`, `SACL`, `SACH`, `SOVM`, `XOR`, `ZAC`, `ZALH`,
  `ZALS`, `SUB`) are primary-transcribed in the opcode research table. The eleven
  ...
  without collisions; the remaining 41 instructions and full reserved-region
...
- **Notes:** Independent model supports `ADD`, `ADDS`, `AND`, `LAC`, `LACK`, `LARK`,
  ...
  and deterministic replay. The eleven common-address data instructions cover
...
- **Notes:** Qualified slice supports the same nineteen instructions as the
  ...
  ADDS/AND/OR/XOR/ZALH/ZALS syntax without a shift operand. The remaining 41
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OV/OVM, and
  144-word internal RAM exist for the nineteen-instruction slice. `LAC`
  ...
  eleven common-address data instructions verify direct/indirect read/write
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of nineteen supported one-cycle instructions, including all
  eleven internal-data operations, on its falling-edge sample and aligns
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all nineteen
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `ADD`/`ADDS`/`AND`/`LAC`/`OR`/`SACL`/`SACH`/`SUB`/`XOR`/`ZALH`/`ZALS` model/RTL tests
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all eleven qualified
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  ...
  `ADD`/`ADDS`/`AND`/`LAC`/`OR`/`SUB`/`XOR`/`ZALH`/`ZALS` reads,
...
- **Notes:** Nineteen-instruction RTL, phase engine, and 144-word RAM are
