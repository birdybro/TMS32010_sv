...
- **Notes:** Thirteen encodings (`LAC`, `LACK`, `LARK`, `LARP`, `LDPK`, `NOP`,
  `ROVM`, `SACL`, `SACH`, `SOVM`, `ZAC`, `ZALH`, `ZALS`) are
  primary-transcribed in the opcode research table. The five common-address
  data instructions add conditional legality constraints for indirect control
  bits; SACH additionally restricts its sparse shift field to 0, 1, and 4. The
  decoder exhaustively classifies all 65,536 words against this partial set
  without collisions; the remaining 47 instructions and full reserved-region
...
- **Notes:** Independent model supports `LAC`, `LACK`, `LARK`, `LARP`, `LDPK`,
  `NOP`, `ROVM`, `SACL`, `SACH`, `SOVM`, `ZAC`, `ZALH`, and `ZALS`, raw
  program loading, logical program/data traces, reset-boundary effects, and
  deterministic replay. The five common-address data instructions cover
  ...
  zero loads cover high-half placement and low-half zero extension.
...
- **Notes:** Qualified slice supports the same thirteen instructions as the
  ...
  ZALH/ZALS common address syntax without a shift operand. The remaining 47
...
- **Notes:** Initial 32-bit accumulator, two 16-bit ARs, ARP, DP, OVM, and
  144-word internal RAM exist for the thirteen-instruction slice. `LAC` verifies
  ...
  five common-address data instructions verify direct/indirect read/write
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of thirteen supported one-cycle instructions, including all
  five internal-data operations, on its falling-edge sample and aligns
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all thirteen
...
- **Notes:** Primary documentation establishes that ordinary operands are
  ...
  `LAC`/`SACL`/`SACH`/`ZALH`/`ZALS` model/RTL tests cover valid address
  selection, logical read/write traces, ordering, and explicitly trap
...
- **Notes:** First control/immediate slice (`LACK`, `NOP`, `ZAC`, `ROVM`,
  ...
  transfer from unsigned low-half zero extension. The rest of the load/store
  family remains. Maintain one subtask per family when implementation begins.
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all five qualified
  internal-data instructions is asserted through the partial native-phase
...
- **Notes:** Seed `0x32010` runs 512 supported instructions with model/RTL
  state, logical-cycle, `LAC`/`ZALH`/`ZALS` reads, `SACL`/`SACH` writes, and
  final 144-word RAM agreement over an identical deterministic image. MAME
...
- **Notes:** Thirteen-instruction RTL, phase engine, and 144-word RAM fit
  Cyclone V in 1,466 ALMs/2,420 registers with no RAM/DSP blocks, 72.31 MHz
  worst slow-corner internal Fmax, and positive 50 MHz setup/hold slack. All
  harness exclusions are enumerated and TimeQuest reports zero unconstrained
  categories; this is still not wrapper I/O closure. Yosys 0.33 passes
  structural/generic synthesis in isolated Ubuntu 24.04 with 6,248 generic
