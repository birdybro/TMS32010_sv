...
- **Notes:** Thirty-six encodings (`ADD`, `ADDS`, `AND`, `APAC`, `DINT`, `DMOV`, `EINT`, `LAC`, `LACK`, `LAR`,
  ...
  fixtures must not be generated by the assembler.
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  multi-cycle, and interrupt control do not exist yet. PUSH/POP stack state is
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  timing is transcribed. One-cycle retirement for all twenty-one qualified
  ...
  sequence remains open under `OQ-016`. Electrical delays are wrapper
