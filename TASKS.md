...
- **Notes:** Thirty-seven encodings (`ADD`, `ADDS`, `AND`, `APAC`, `DINT`, `DMOV`, `EINT`, `LAC`, `LACK`, `LAR`,
  ...
  accepting 19,051 supported words without collisions; the remaining 23
...
- **Notes:** Qualified slice supports the same thirty-seven instructions as the
  ...
  PAC/APAC/SPAC/DINT/EINT. The remaining 23
...
- **Notes:** Initial 32-bit accumulator, 16-bit T register, 32-bit P register,
  ...
  thirty-seven-instruction slice. `LAC`
...
- **Notes:** Temporary one-enable instruction execution and
  ...
  retires each of thirty-seven supported one-cycle instructions, including all
  twenty-two internal-data operations, on its falling-edge sample and aligns
  PC/native address across stalls, traps, and reset. General overlap, branch,
  multi-cycle, and interrupt control do not exist yet. SUBC tests use a
...
- **Notes:** Appendix A normal read and table-transfer pin waveforms are
  ...
  release delay. A partial wrapper integrates those phases with all thirty-seven
...
- **Notes:** Normal memory read, TBLR/TBLW, IN/OUT, reset, INT, and BIO pin
  ...
  ownership remain. PUSH/POP are primary-confirmed as one-word/two-cycle
...
- **Notes:** Thirty-seven-instruction RTL, phase engine, multiplier, and
  144-word RAM are
